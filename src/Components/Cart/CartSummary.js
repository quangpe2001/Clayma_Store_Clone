...
export default function CartSummary(props) {
  ...
  let amountSaved = 0;
  myShoppingCart.map(
    (product) => (cartTotal += product.price * product.product_selected_qty)
  );
  ...
  myShoppingCart.map(
    (product) =>
      (amountSaved +=
        product.discount > 0
          ? (product.price * product.product_selected_qty * product.discount) /
            100
          : 0)
  );
          ...
          <h4 className="cart-summary-amount-align-center">
            <span>
              {new Intl.NumberFormat("en-US", {
                style: "currency",
                currency: "ZAR",
              }).format((cartTotal / vat_percentage - cartTotal) * -1)}
            </span>
          </h4>
        </div>
      </div>
          ...
          <h3 className="cart-summary-amount-align-center">
            <span className="product-price-after-discount">
              {new Intl.NumberFormat("en-US", {
                style: "currency",
                currency: "ZAR",
              }).format(cartTotal + estimatedShippingCost - amountSaved)}
            </span>
          </h3>
        </div>
      </div>
